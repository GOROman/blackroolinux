...
void __init setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = command_line;
	...
	/* BLACKROO: fixed initrd detection (was marked "Fixme, I'm broken")
	 * Search for INRD magic header after kernel _end.
	 */
	{
		unsigned long tmp;
		unsigned long *initrd_header;
		/* _end already declared at line 103 as extern int _end */

		tmp = (((unsigned long)&_end + PAGE_SIZE-1) & PAGE_MASK) - 8;
		if (tmp < (unsigned long)&_end)
			tmp += PAGE_SIZE;
		initrd_header = (unsigned long *)tmp;
		printk("Blackroo: checking for initrd at 0x%08lx\n", tmp);
		if (initrd_header[0] == 0x494E5244) {
			...
			printk("No initrd found (magic=0x%08lx)\n", initrd_header[0]);
		}
	}
#endif
