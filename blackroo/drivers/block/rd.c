			   ...
			   size_t count, loff_t *ppos)
{
	...
	if (count == 0) return 0;
	/* BLACKROO: use memcpy instead of copy_to_user.
	 * On uClinux (no MMU), copy_to_user may fail when the
	 * destination is a kernel buffer (called with KERNEL_DS
	 * from rd_load_image). memcpy works directly.
	 * Original: copy_to_user(buf, (char *)initrd_start + *ppos, count);
	 */
	memcpy(buf, (char *)initrd_start + *ppos, count);
	*ppos += count;
	return count;
}
...
{
	printk("RAMDISK: rd_open(minor %d)\n", DEVICE_NR(inode->i_rdev));
	 ...
	 */
	/*
	 * BLACKROO 2026-08-21: do not fail the open when there is no i_bdev.
	 *
	 * This block is only the "immunize against invalidate_buffers() and
	 * prune_icache()" optimisation. It used to `return -ENXIO` when
	 * inode->i_bdev was NULL - but blkdev_get(), which is how mount_root()
	 * opens the root device, passes a fake inode with no i_bdev. So the
	 * root device could never be opened:
	 *
	 ...
	 *
	 * Upstream 2.4 does this with bdget() rather than relying on the
	 * inode. Here it is enough to skip the immunisation when there is no
	 * inode to grab: the ramdisk's buffers are pinned with
	 * mark_buffer_protected() anyway (see rd_make_request).
	 */
	if (rd_inode[DEVICE_NR(inode->i_rdev)] == NULL && inode->i_bdev) {
...
identify_ramdisk_image(kdev_t device, struct file *fp, int start_block)
{
	...
	/* BLACKROO: debug what we read from initrd device */
	printk("RAMDISK: blk%d read: %02x %02x %02x %02x %02x %02x %02x %02x"
	       " initrd_start=0x%lx fop=%p\n",
	       start_block,
	       (unsigned char)buf[0], (unsigned char)buf[1],
	       (unsigned char)buf[2], (unsigned char)buf[3],
	       (unsigned char)buf[4], (unsigned char)buf[5],
	       (unsigned char)buf[6], (unsigned char)buf[7],
	       initrd_start, fp->f_op);
	...
	/* BLACKROO: debug block 1 — ext2 superblock magic is at buf[0x38] */
	printk("RAMDISK: blk%d read: %02x %02x %02x %02x @0x38: %02x %02x\n",
	       start_block + 1,
	       (unsigned char)buf[0], (unsigned char)buf[1],
	       (unsigned char)buf[2], (unsigned char)buf[3],
	       (unsigned char)buf[0x38], (unsigned char)buf[0x39]);
...
{
	/*
	 * BLACKROO: bypass the broken initrd->ramdisk copy path.
	 * The rd_load_image() function opens /dev/initrd as a block device
	 * but the read path doesn't work correctly on uClinux (no MMU).
	 *
	 * Instead, directly memcpy the initrd data into the ramdisk buffer.
	 * The ramdisk pages are allocated on first access, so we write
	 * through the block device write path which DOES work.
	 *
	 * Original: rd_load_image(MKDEV(MAJOR_NR, INITRD_MINOR),rd_image_start,0);
	 */
	if (initrd_start && initrd_end > initrd_start) {
		/*
		 * BLACKROO 2026-08-21: copy the initrd straight into the
		 * buffer cache, which is where this ramdisk keeps its data
		 * (see rd_make_request: getblk + mark_buffer_protected).
		 *
		 * The previous version went through def_blk_fops->write() and
		 * ignored the return value, so it printed "initrd copied"
		 * whether or not anything was stored - and nothing was: a
		 * later bread() of block 1 returned uninitialised slab memory
		 * and the mount failed with "Unable to mount root fs".
		 *
		 * A monitor round-trip test (getblk, write pattern, bread)
		 * showed the device itself works, so this uses that same path
		 * directly, and verifies the result instead of assuming it.
		 */
		unsigned long size = initrd_end - initrd_start;
		unsigned long nblocks = (size + BLOCK_SIZE - 1) / BLOCK_SIZE;
		unsigned char *src = (unsigned char *)initrd_start;
		unsigned long i;
		int failed = 0;

		printk("BLACKROO: copying initrd %lu bytes (%lu blocks) "
		       "into /dev/ram0\n", size, nblocks);

		for (i = 0; i < nblocks; i++) {
			struct buffer_head *bh;
			unsigned long chunk = size - i * BLOCK_SIZE;

			if (chunk > BLOCK_SIZE)
				chunk = BLOCK_SIZE;

			bh = getblk(MKDEV(MAJOR_NR, 0), i, BLOCK_SIZE);
			if (!bh) {
				printk("BLACKROO: getblk failed at block %lu\n", i);
				failed = 1;
				break;
			}

			memcpy(bh->b_data, src + i * BLOCK_SIZE, chunk);
			if (chunk < BLOCK_SIZE)
		...
		if (!failed) {
			struct buffer_head *chk;

			chk = bread(MKDEV(MAJOR_NR, 0), 1, BLOCK_SIZE);
			if (chk) {
				printk("BLACKROO: initrd in ram0, superblock "
				       "magic %02x%02x %s\n",
				       (unsigned char)chk->b_data[0x39],
				       (unsigned char)chk->b_data[0x38],
				       ((unsigned char)chk->b_data[0x38] == 0x53 &&
					(unsigned char)chk->b_data[0x39] == 0xef)
				       ? "(ext2 OK)" : "(NOT ext2)");
				brelse(chk);
			} else {
				printk("BLACKROO: readback of ram0 failed\n");
			}
		}
	} else {
