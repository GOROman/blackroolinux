...
asmlinkage ssize_t sys_write(unsigned int fd, const char * buf, size_t count)
{
	...
	/* BLACKROO: debug userspace writes from init (pid 1) */
	if (current->pid <= 2 && fd <= 2 && count > 0 && count < 256) {
		char dbuf[64];
		int dlen = count > 60 ? 60 : count;
		memcpy(dbuf, buf, dlen);
		dbuf[dlen] = 0;
		printk("BLACKROO write(%d, \"%s\", %d) pid=%d\n", fd, dbuf, count, current->pid);
	}
