...
static int load_flat_binary(struct linux_binprm * bprm, struct pt_regs * regs)
{
   ...
   /* BLACKROO: debug FLAT binary loading */
   printk("BLACKROO FLAT: rev=%lu entry=%lu text=%lu data=%lu-%lu bss=%lu stack=%lu reloc=%lu/%lu flags=0x%lx\n",
      hdr->rev, hdr->entry_point, hdr->text_start,
      hdr->data_start, hdr->data_end, hdr->bss_end,
      hdr->stack_size, hdr->reloc_start, hdr->reloc_count, hdr->flags);
   ...
   /* BLACKROO: dump what's at the entry point before jumping */
   {
      unsigned long entry = current->mm->start_code + hdr->entry_point;
      unsigned long *code = (unsigned long *)entry;
      printk("BLACKROO FLAT: entry=0x%lx stack=0x%lx\n", entry,
         (long)(current->mm->start_stack));
      printk("BLACKROO FLAT: code[0]=0x%08lx code[1]=0x%08lx code[2]=0x%08lx code[3]=0x%08lx\n",
         code[0], code[1], code[2], code[3]);
      /* Expected: code[0]=0x27bdffe0 (addiu sp,sp,-32) for our asm init */
      printk("BLACKROO FLAT: codep=0x%lx text_start=%lu start_code=0x%lx\n",
         codep, hdr->text_start, current->mm->start_code);
   }
