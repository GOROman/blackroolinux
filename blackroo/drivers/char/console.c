...
#include <asm/ps/sio.h>	/* BLACKROO: SIO1 mirror, see blackroo_con_mirror() */
#endif
...
#ifdef CONFIG_VT_SCREEN_DEV
extern void vcs_make_devfs (unsigned int index, int unregister);
#else
/* BLACKROO: /dev/vcs is not built (CONFIG_VT_SCREEN_DEV off), and with devfs
 * off as well this was already a call into an empty function. See docs/28. */
static inline void vcs_make_devfs (unsigned int index, int unregister) { }
#endif
...
/*
 * BLACKROO: mirror tty writes to SIO1.
 *
 * printk already reaches both the GPU (vt_console_print) and the serial port
 * (siocons in arch/mipsnommu/ps/siocon.c), but a *userspace* write goes
 * through one tty only. Now that /dev/console resolves to the VT, everything
 * a process prints would land on the TV and nowhere else - and the serial
 * link is still how this machine is actually driven.
 *
 * So do_con_write() copies each chunk out of SIO1 as well. The hook is here
 * and not in vt_console_print(), which is deliberate: printk does NOT come
 * through do_con_write(), so serial sees each kernel message exactly once.
 *
 * Polled, bounded, and called before the console spinlock is taken - the wait
 * is ~87us per byte at 115200 and must not run with interrupts off.
 */
#ifdef CONFIG_GPUPSX_CONSOLE

#define BLACKROO_TX_SPINS 400000

static void blackroo_sio_putc(int c)
{
	long spins = 0;
	...
	while ((inw(SIO_STAT_REG) & SIO_RFW) == 0)
		if (++spins > BLACKROO_TX_SPINS)
			return;		/* nothing listening - do not wedge */

	outb(c, SIO_DATA_REG);
}

static void blackroo_con_mirror(const unsigned char *buf, int count)
{
	int i;

	for (i = 0; i < count; i++)
		blackroo_sio_putc(buf[i]);
}
#else
#define blackroo_con_mirror(b, c) do { } while (0)
#endif /* CONFIG_GPUPSX_CONSOLE */
...
again:
		...
		/* BLACKROO: on uClinux (no MMU), user and kernel share the
		 * same address space. copy_from_user's assembly implementation
		 * may fail on some address ranges. Just memcpy instead.
		 * Original: copy_from_user(con_buf, buf, count) */
		memcpy(con_buf, buf, count);
#else
	...
	/* BLACKROO: same bytes out of the serial port. Before the spinlock -
	 * this polls, and must not run with interrupts disabled. */
	blackroo_con_mirror(buf, count);
...
/*
 * BLACKROO 2026-08-21: this was `name: "ttyS"`, which is what kept userspace
 * off the TV.
 *
 * console_setup() records every `console=` in console_cmdline[] and points
 * preferred_console at the LAST one, so with
 *     console=ttyS0,115200 console=tty0
 * the preferred entry is `tty` index 0. register_console() only sets
 * CON_CONSDEV when a console's *name* matches the preferred entry - and with
 * this named "ttyS" it matched entry 0 (the serial one) instead, so nothing
 * ever got CON_CONSDEV. `console_drivers` then kept whatever registered first,
 * which is siocons (console_init runs before con_init).
 *
 * tty_open() resolves /dev/console by walking that list and calling the head's
 * ->device(), so /dev/console became ttyS0 - the serial tty - and userspace
 * output never reached the GPU at all. With the name corrected, `console=tty0`
 * matches, this console takes CON_CONSDEV and the head of the list, and
 * vt_console_device() sends /dev/console to tty1: the VT, drawn by
 * psxvga_con on the GPU.
 */
