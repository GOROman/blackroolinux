...
#include <linux/random.h>	/* add_blkdev_randomness(), used below */
...
/* BLACKROO: add_blkdev_randomness() comes from <linux/random.h> above, which
 * turns it into an empty inline when CONFIG_DEV_RANDOM is off. The second
 * plain declaration that used to sit here defeated that inline, and bu.c
 * failed to link against a random.o that is no longer built. */
