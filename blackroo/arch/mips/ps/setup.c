...
/* BLACKROO: stubs for missing symbols */
#include <linux/mc146818rtc.h>

struct rtc_ops no_rtc_ops = {0};

long sys_ioperm(unsigned long from, unsigned long num, int on) { return -ENOSYS; }
long sys_iopl(unsigned int level) { return -ENOSYS; }

int page_is_ram(unsigned long pagenr) { return 1; }
